...
// Copyright (c) 2009-2017 Verge
// Copyright (c) 2018 CrypticCoin
...
bool CTxDB::LoadBlockIndex(CClientUIInterface* uiInterface)
{
    ...
    for (CBlockIndex* pindex = pindexBest; pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel>0 && !block.CheckBlock(verifier, true, true, (nCheckLevel>6)))
        {
                        ...
                        BOOST_FOREACH(const CDiskTxPos &txpos, txindex.vSpent)
                        {
                            if (!txpos.IsNull())
                            {
                                ...
                                if (nCheckLevel>5)
                                {
                                    ...
                                    else if (!txSpend.CheckTransaction(verifier))
                                    {
