                ...
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                            *pcrypticcoinParams, wtxItem.second.joinSplitPubKey);
...
bool CWallet::FundTransactionWithChange(CMutableTransaction& tx, CAmount &nFeeRet, int& nChangePosRet, std::string change,std::string& strFailReason){
    vector<CRecipient> vecSend;
    ...
    bool ignoreLocked)
{
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapSproutNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pcrypticcoinParams, wtx.joinSplitPubKey);
            try {
