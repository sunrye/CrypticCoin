...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in CrypticCoin
    bool fStrictPayToScriptHash = true; // Always active in CrypticCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CrypticCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hashPoW > hashTarget && pblock->IsProofOfWork())
        return error("CrypticCoinMiner : proof-of-work not meeting target");

    uint256 hashBlock = pblock->GetHash();

    //// debug print
    printf("CrypticCoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CrypticCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("CrypticCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadCrypticCoinMiner(void* parg);
...
void CrypticCoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("CrypticCoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running CrypticCoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadCrypticCoinMiner(void* parg)
{
    ...
    try
    {
        ...
        CrypticCoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadCrypticCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadCrypticCoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadCrypticCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d CrypticCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadCrypticCoinMiner, pwallet))
                printf("Error: NewThread(ThreadCrypticCoinMiner) failed\n");
            MilliSleep(10);
        }
    }
}
