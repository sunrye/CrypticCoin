...
public:
    AsyncRPCOperation_sendmany(CTransaction contextualTx, std::string fromAddress, std::vector<SendManyRecipient> tOutputs, std::vector<SendManyRecipient> zOutputs, int minDepth, CAmount fee = ASYNC_RPC_OPERATION_DEFAULT_MINERS_FEE, Value contextInfo = Value::null);
...
private:
    ...
    Value perform_joinsplit(AsyncJoinSplitInfo &);

    // JoinSplit with input notes to spend (JSOutPoints))
    Value perform_joinsplit(AsyncJoinSplitInfo &, std::vector<JSOutPoint> & );

    // JoinSplit where you have the witnesses and anchor
    Value perform_joinsplit(
        AsyncJoinSplitInfo & info,
        std::vector<boost::optional < ZCIncrementalWitness>> witnesses,
        uint256 anchor);

    void sign_send_raw_transaction(Value obj);     // throws exception if there was an error
...
public:
    ...
    Value perform_joinsplit(AsyncJoinSplitInfo &info) {
        return delegate->perform_joinsplit(info);
    }

    Value perform_joinsplit(AsyncJoinSplitInfo &info, std::vector<JSOutPoint> &v ) {
    ...
    void sign_send_raw_transaction(Value obj) {
