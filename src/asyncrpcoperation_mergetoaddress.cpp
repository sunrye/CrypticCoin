...
int mta_find_output(Value obj, int n)
{
    ...
    recipient_(recipient), fee_(fee), contextinfo_(contextInfo)
{
    ...
    printf("zrpc %s: z_mergetoaddress initialized\n", getId());
...
bool AsyncRPCOperation_mergetoaddress::main_impl()
{
    ...
    printf("%s %s: spending %s to send %s with fee %s\n", isPureTaddrOnlyTx ? "zrpc" : "zrpcunsafe",
        getId(), FormatMoney(targetAmount), FormatMoney(sendAmount), FormatMoney(minersFee));
    printf("zrpc %s: transparent input: %s\n", getId(), FormatMoney(t_inputs_total));
    printf("zrpcunsafe %s: private input: %s\n", getId(), FormatMoney(z_inputs_total));
    if (isToTaddr_) {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            printf("zrpcunsafe %s: generating note for %s (amount=%s)\n",
                getId(),
                outputType,
 ...
 */
void AsyncRPCOperation_mergetoaddress::sign_send_raw_transaction(Value obj)
{
...
Value AsyncRPCOperation_mergetoaddress::perform_joinsplit(MergeToAddressJSInfo& info)
{
    std::vector<boost::optional<ZCIncrementalWitness>> witnesses;
    uint256 anchor;
    {
...
Value AsyncRPCOperation_mergetoaddress::perform_joinsplit(MergeToAddressJSInfo& info, std::vector<JSOutPoint>& outPoints)
{
    std::vector<boost::optional<ZCIncrementalWitness>> witnesses;
    uint256 anchor;
    {
...
Value AsyncRPCOperation_mergetoaddress::perform_joinsplit(
    MergeToAddressJSInfo& info,
    std::vector<boost::optional<ZCIncrementalWitness>> witnesses,
    uint256 anchor)
{
 ...
 */
Value AsyncRPCOperation_mergetoaddress::getStatus() const
{
